...
const DB_CONFIGS = [
  ...
  {
    host: '10.0.2.1', // Gateway red Coolify (puerto expuesto 3307)
    port: 3307,
  },
  {
  ...
  {
    host: '172.17.0.1', // Gateway docker0
    port: 3307,
  },
  {
    host: '127.0.0.1',
...
export function initDB() {
  ...
  dbInitPromise = (async () => {
    try {
      ...
      // Asegurar índices en tablas existentes
      try {
        await activePool.query('ALTER TABLE tours ADD INDEX idx_created_at (created_at)');
      } catch (e) {}
      try {
        await activePool.query('ALTER TABLE tours ADD INDEX idx_destination (destination)');
      } catch (e) {}

      // Aumentar sort_buffer_size para la sesión actual si es posible
      try {
        await activePool.query('SET SESSION sort_buffer_size = 4194304');
      } catch (e) {}
      ...
      `;
      await activePool.query(createConfigTableQuery);
...
export async function fetchToursFromDB(destination = 'all') {
  try {
    ...
    let rows;
    try {
      // Intentar consulta ordenada con MySQL
      const [orderRows] = await activePool.query(queryStr + ' ORDER BY created_at DESC', params);
      rows = orderRows;
    } catch (sortErr) {
      console.warn('MySQL ORDER BY failed, falling back to in-memory sort:', sortErr.message);
      // Fallback: consulta sin ORDER BY y ordenamiento en memoria de Node.js
      const [plainRows] = await activePool.query(queryStr, params);
      rows = plainRows;
...
export async function fetchGeneralConfigFromDB() {
  try {
    await initDB();
    const activePool = await getWorkingPool();
    const [rows] = await activePool.query('SELECT data FROM site_config WHERE id = ?', ['general_config']);
    if (rows && rows.length > 0) {
      return parseJSON(rows[0].data);
    }
  } catch (error) {
    console.warn('Fetch general config from MySQL failed:', error.message);
  }
  return null;
}

export async function saveGeneralConfigToDB(configData) {
  await initDB();
  const activePool = await getWorkingPool();
  try {
    const queryStr = `
      INSERT INTO site_config (id, data) VALUES ('general_config', ?)
      ON DUPLICATE KEY UPDATE data = VALUES(data);
    `;
    await activePool.query(queryStr, [JSON.stringify(configData)]);
    return true;
  } catch (error) {
    console.error('Error saving general config to MySQL DB:', error);
    throw error;
  }
}
...
export async function getWhatsAppClickCount() {
  try {
    await initDB();
    const activePool = await getWorkingPool();
    const [rows] = await activePool.query('SELECT data FROM site_config WHERE id = ?', ['whatsapp_clicks']);
    if (rows && rows.length > 0) {
      const parsed = parseJSON(rows[0].data);
      return Number(parsed.count) || 0;
    }
  } catch (e) {
    console.warn('getWhatsAppClickCount error:', e.message);
  }
  return 0;
}

export async function incrementWhatsAppClickCount() {
  await initDB();
  const activePool = await getWorkingPool();
  try {
    const queryStr = `
      INSERT INTO site_config (id, data)
      VALUES ('whatsapp_clicks', JSON_OBJECT('count', 1))
      ON DUPLICATE KEY UPDATE
        data = JSON_SET(data, '$.count', COALESCE(CAST(JSON_UNQUOTE(JSON_EXTRACT(data, '$.count')) AS UNSIGNED), 0) + 1);
    `;
    await activePool.query(queryStr);
    return true;
  } catch (e) {
    console.error('incrementWhatsAppClickCount error:', e);
    return false;
  }
}
