...
export function getWhatsAppLink({ phone = INITIAL_CONFIG.whatsappNumber, tourTitle = '', destination = '', customMessage = '' }) {
  const cleanPhone = (phone || INITIAL_CONFIG.whatsappNumber).replace(/[^0-9+]/g, '');
  ...
  if (typeof window !== 'undefined') {
    try {
      if (navigator.sendBeacon) {
        navigator.sendBeacon('/api/stats/whatsapp', '');
      } else {
        fetch('/api/stats/whatsapp', { method: 'POST', keepalive: true }).catch(() => {});
      }
    } catch (e) {
